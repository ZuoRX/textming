...
#导入数据
train <- read.csv("D://例子//lizi.csv")
csv <- train

#安装词典：
installDict("D://例子//词典//law.scel", "law")
installDict("D://例子//词典//lawbool.scel", "lawbool")
installDict("D://例子//词典//medicine.scel", "med")
...
nvword<-function(wf)
{
  ...
  for(i in 1:length(wf)){
    ...
    wf.n[[i]]<-wf[[i]][which(names(wf[[i]])=='n'|names(wf[[i]])=='v'|names(wf[[i]])=='lawbook'|names(wf[[i]])=='nz'|names(wf[[i]])=='med'|names(wf[[i]])=='law')]#剔除不是这些词的东西
    wf.n[[i]]<-wf.n[[i]][which(is.na(match(wf.n[[i]],c("是","有","可以"))))]#初步降维 [[]]和[]的区别只取到内容，不取到变量名
...
summary(mdc)
modelky5<-names(mdc[which(mdc>2)])
...
wf1<-segword(train)
ovid1<-nvword(wf1)
prtn1<-prepword(modelky5,ovid1,train)  
traindata<-prtn1[[1]]
pdoc1<-prtn1[[2]]
zz<-c(1:length(train[,1]))
qyn1<-train[which(is.na(match(zz,pdoc1))),]$type
...
wf2<-segword(test)
ovid2<-nvword(wf2)
prtn2<-prepword1(modelky5,ovid2,test)
testdata<-prtn2[[1]]
pdoc2<-prtn2[[2]]
tt<-c(1:length(test[,1]))
qyn2<-test[which(is.na(match(tt,pdoc2))),]$type
