...
# library(devtools)
# install_github("qinwf/cidian")#这个包没下好

#LDA主题模型（topic model）Latent Dirichlet Allocation
#一种能够从大量文本中提取出主题的概率模型  主题发现、文档标记
...
#names(guangzhou)<-c("job","place","company","post date","welfare","salary","duties")

guangzhou_duty<-guangzhou[,c(1,7)]
#删除重复项
guangzhou_duty<-guangzhou_duty[!duplicated(guangzhou_duty),]
names(guangzhou_duty)<-c("job","duties")
write.table(guangzhou_duty$duties,"c:/users/lenovo/desktop/LDA/guangzhou.txt")
...
comments_seg <- cutter["c:/users/lenovo/desktop/LDA/guangzhou.txt"] 
...
# 即让p(wi|topici)发挥的作用小，
# 这样p(di|topici)发挥的作用就大。其中的β，调大的结果是让p(di|topici)发挥的作用变下，
